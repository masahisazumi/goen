...
export default function EditSpacePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchSpace = async () => {
      try {
        ...
        if (space.tags) {
          try {
            setSelectedTags(JSON.parse(space.tags));
          } catch {
            setSelectedTags([]);
          }
        }
        if (space.facilities) {
          try {
            setSelectedFacilities(JSON.parse(space.facilities));
          } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || "スペースの更新に失敗しました");
        return;
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json();
        setError(data.error || "スペースの削除に失敗しました");
        return;
      }
  ...
  if (!session) {
    router.push(`/login?callbackUrl=/spaces/${id}/edit`);
    return null;
  }
            ...
            <>
              <div className="mb-8 flex items-center justify-between">
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>スペースを削除しますか？</AlertDialogTitle>
                      <AlertDialogDescription>
                        この操作は取り消せません。スペースに関連するすべてのデータ（お気に入り、レビューなど）も削除されます。
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      ...
                      {existingImages.map((img) => (
                        <div key={img.id} className="relative aspect-square rounded-xl overflow-hidden bg-gray-100">
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                await fetch(`/api/upload/${img.id}?type=space`, { method: "DELETE" });
